...
function getContentfulFileUrl(fileContentType) {
  return `https:${fileContentType.fields.file.url}`;
}
...
export async function getHeroImage(params) {
  ...
  return {
    ...
    heroImage: getContentfulFileUrl(item.fields.heroImage),
  };
}
...
export async function getIntroductionSection(params) {
  ...
  console.log(
    "🚀 ~ file: api.js ~ line 46 ~ getIntroductionSection ~ item",
    items
  );

  return items.map((item) => ({
    image: getContentfulFileUrl(item.fields.image),
    title: item.fields.title,
...
export async function getAboutPage(params) {
  ...
  return {
    title: section.fields.title,
    aboutSection: {
      image: getContentfulFileUrl(section.fields.aboutSection.fields.image),
...
export async function getJoinUsPage(params) {
  ...
  console.log(section.fields, "read this");

  return {
    heading: section.fields.heading,
    videoClip: getContentfulFileUrl(section.fields.videoClip),
    whyStretchSection: {
