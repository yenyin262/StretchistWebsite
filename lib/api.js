...
// notes here
// client here is used to interact with contentful api
const client = createClient({
  space: process.env.CF_SPACE_ID,
  accessToken: process.env.CF_DELIVERY_ACCESS_TOKEN,
});

// previewClient is interacting with api is preview form
// preview how content looks before being publishing
...
function getContentfulFileUrl(fileContentType) {
  if (
    fileContentType.fields &&
    fileContentType.fields.file &&
    fileContentType.fields.file.contentType
  ) {
    const contentType = fileContentType.fields.file.contentType;
    let type = "image";
...
// this function is an example of interacting with contentful API
...
// console.log(
//   "🚀 ~ file: api.js ~ line 61 ~ const{items}=awaitgetClient ~ items",
//   items[0].fields.media
// );
...
export async function getIntroductionSection(params) {
  ...
  console.log(
    "🚀 ~ file: api.js ~ line 90 ~ const{items}=awaitgetClient ~ items",
    items
  );
  ...
  return items.map((item) => ({
    ...
    media: getContentfulFileUrl(item.fields.media),
  }));
}
